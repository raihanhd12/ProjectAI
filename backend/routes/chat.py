...
async def get_chat_history(
    ...
    try:
        ...
        if not session_id:
            recent_sessions = chat_db.get_recent_chat_sessions(1)
            if recent_sessions:
                session_id = recent_sessions[0][0]
            else:
                return {"messages": [], "session_id": None}
        
        messages = chat_db.get_chat_history(session_id)
...
async def get_chat_sessions(
    ...
    """
    try:
        sessions = chat_db.get_recent_chat_sessions(limit)
        formatted_sessions = []
...
async def create_chat_session(
    ...
    try:
        ...
        new_session_id = chat_db.create_session(session_id)
...
async def delete_chat_session(
    ...
    """
    try:
        success = chat_db.delete_chat_session(session_id)
        if success:
...
async def query_chat(
    ...
    try:
        ...
        chat_db.save_message(session_id, "user", chat_query.prompt)
        ...
        if query_results and len(query_results.get("documents", [[]])[0]) > 0:
            ...
            relevant_text, relevant_text_ids = rag_model.re_rank_documents(chat_query.prompt, context)
