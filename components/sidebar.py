...
from models.rag import LocalRAG  # Import LocalRAG directly
...
def display_sidebar(app_modes, DB_DIR, DATA_DIR):
    ...
    if st.sidebar.button("New Conversation"):
        new_id = ChatHistory.add_conversation(app_mode=st.session_state.app_mode)
        st.session_state.current_conversation_id = new_id
        st.rerun()
                ...
                if st.button("🗑️", key=f"del_{conv_id}"):
                    ...
                    if st.session_state.current_conversation_id == conv_id:
                        if len(conversation_ids) > 1:
                            ...
                            other_ids = [cid for cid in conversation_ids if cid != conv_id]
    ...
    if hasattr(st.session_state, "rag_system"):
        ...
        st.sidebar.caption(f"Embeddings: {st.session_state.rag_system.embedding_model_name.split('/')[-1]}")
...
# Embedding model selection
    ...
    if selected_llm != st.session_state.llm_model:
        ...
        if "rag_system" in st.session_state:
            # Update LLM model
            try:
                st.session_state.rag_system.llm = OllamaLLM(model=selected_llm)
                st.session_state.rag_system.model_name = selected_llm
